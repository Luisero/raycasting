...
// Implementação fictícia de getNormal (quem responde a normal é o triângulo interno)
Vector4 Mesh::getNormal(Point collide) {
...
Color Mesh::shade(const Ray& viewingRay, const Point& P, const Point& lightPosition, const Color& lightIntensity, const Color& ambientLightIntensity, const Point& observerPosition, const std::vector<std::unique_ptr<Object>>& allObjects) {
    
    // Precisamos descobrir QUAL triângulo foi atingido para pegar a normal dele.
    // Como intersect() só retorna float, refazemos o teste rápido aqui (seguro para Threads)
    
    float closest_t = -1.0f;
    Triangle* hitTri = nullptr;

    // Otimização: Sabemos que P está no triângulo, então t deve ser a distância até P
    float distToP = (P - viewingRay.origin).lenght();

    for (const auto& tri : triangles) {
        float t = tri->intersect(viewingRay);
        // Verifica se este é o triângulo que gerou o ponto P (com pequena margem de erro)
    ...
    if (hitTri) {
        // Deixa o triângulo calcular a cor (Phong, Sombra, etc)
        return hitTri->shade(viewingRay, P, lightPosition, lightIntensity, ambientLightIntensity, observerPosition, allObjects);
    }
    ...
    while (std::getline(file, line)) {
        ...
        if (prefix == "v") {
            ...
            auto getIndex = [](const std::string& s) -> int {
                size_t slash = s.find('/');
                if (slash == std::string::npos) return std::stoi(s);
            ...
            if (i1 > 0 && i2 > 0 && i3 > 0 && i1 <= (int)temp_verts.size() && i2 <= (int)temp_verts.size() && i3 <= (int)temp_verts.size()) {
                Point p1 = temp_verts[i1 - 1];
                Point p2 = temp_verts[i2 - 1]; // Corrigido typo temp_vertices -> temp_verts
...
void Mesh::calculateBounds() {
    ...
    // Adiciona uma margem de segurança (epsilon) para evitar caixas com espessura zero
